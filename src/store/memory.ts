...
export class MemoryStore implements Store {
  ...
  async init(): Promise<void> {}
  async close(): Promise<void> { this.rateLimitMap.clear(); this.requestLogs = []; }
  async migrate(): Promise<void> {}

  async rateLimitGet(key: string): Promise<RateLimitEntry | null> {
    const item = this.rateLimitMap.get(key);
    if (!item || item.expiresAt < Date.now()) { this.rateLimitMap.delete(key); return null; }
    return item.entry;
  }
  ...
  async logRequest(entry: RequestLogEntry): Promise<void> { this.requestLogs.push(entry); }
